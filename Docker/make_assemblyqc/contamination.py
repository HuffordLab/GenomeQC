...
megablast_output = output_name + "_megablast" #name of the megablast output file

#run megablast on the input genome assembly
cline = NcbiblastnCommandline(cmd='blastn', task="megablast", query=sys.argv[1], db="/opt/contamination_check/Univec_modified.fasta", outfmt=6, max_target_seqs=1, max_hsps=1, num_threads=4, evalue=1e-25, out=megablast_output)
...
if file_s == 0:
  ...
  def mapping():
      ...
      map_f = "/opt/contamination_check/taxon_n"  #mapping file (TSV format), in which one column lists a sequence ID (of a subject) and another the NCBI TaxID
  ...
  ##name of the output plot file
  contamination_plot_name = output_name + "_contamination_plot.html"
  fig = go.Figure(data=data, layout=layout)
  offline.plot(fig, filename=contamination_plot_name, image='png')
